...
(function (root, factory) {
    ...
    var QueryBuilder = function QueryBuilder($el, options) {
    ...
    QueryBuilder.defaults = function (options) {
        if ((typeof options === 'undefined' ? 'undefined' : _typeof(options)) == 'object') {
            ...
            if (_typeof(QueryBuilder.DEFAULTS[options]) == 'object') {
     ...
     * Allowed inputs
     */
    QueryBuilder.inputs = ['text', 'textarea', 'radio', 'checkbox', 'select'];
    ...
    QueryBuilder.prototype.bindEvents = function () {
        ...
        // model events
        this.model.on({
            'drop': function drop(e, node) {
                node.$el.remove();
                self.refreshGroupsConditions();
            },
            'add': function add(e, node, index) {
                if (index === 0) {
            ...
            'move': function move(e, node, group, index) {
                node.$el.detach();
            ...
            'update': function update(e, node, field, value, oldValue) {
    ...
    QueryBuilder.prototype.refreshGroupsConditions = function () {
        (function walk(group) {
            if (!group.flags || group.flags && !group.flags.condition_readonly) {
                group.$el.find('>' + Selectors.group_condition).prop('disabled', group.rules.length <= 1).parent().toggleClass('disabled', group.rules.length <= 1);
            }
    ...
    QueryBuilder.prototype.applyGroupFlags = function (group) {
        ...
        if (flags.condition_readonly) {
            group.$el.find('>' + Selectors.group_condition).prop('disabled', true).parent().addClass('readonly');
        }
        if (flags.no_delete) {
    ...
    QueryBuilder.prototype.displayError = function (node) {
        if (this.settings.display_errors) {
            if (node.error === null) {
                ...
                var error = $.extend([], node.error, [this.lang.errors[node.error[0]] || node.error[0]]);

                node.$el.addClass('has-error').find(Selectors.error_container).eq(0).attr('title', Utils.fmt.apply(null, error));
            }
        }
    };
    ...
    QueryBuilder.prototype.setOptions = function (options) {
        ...
        $.makeArray($(Object.keys(options)).filter(QueryBuilder.modifiable_options)).forEach(function (opt) {
    ...
    QueryBuilder.prototype.translateLabel = function (label) {
        return (typeof label === 'undefined' ? 'undefined' : _typeof(label)) == 'object' ? label[this.settings.lang_code] || label['en'] : label;
    };
...
<select class="form-control" name="{{= it.rule.id }}_operator"> \
  ...
  {{? optgroup !== null }}</optgroup>{{?}} \
        ...
        fields.forEach(function (field) {
            Object.defineProperty(obj.prototype, field, {
                ...
                set: function set(value) {
                    var oldValue = this.__[field] !== null && _typeof(this.__[field]) == 'object' ? $.extend({}, this.__[field]) : this.__[field];
     ...
     * @param {jQuery}
     */
    var Node = function Node(parent, $el) {
     ...
     * @param {jQuery}
     */
    var Group = function Group(parent, $el) {
     ...
     * @param {jQuery}
     */
    var Rule = function Rule(parent, $el) {
...
<div' + style + ' class="' + filter.input + ' ' + filter.input + '-' + color + '"> \
  ...
  <label for="' + id + '">' + val + '</label> \
    ...
    QueryBuilder.define('bt-tooltip-errors', function (options) {
        ...
        this.model.on('update', function (e, node, field) {
            if (field == 'error' && self.settings.display_errors) {
                node.$el.find(Selectors.error_container).eq(0).tooltip(options).tooltip('hide').tooltip('fixTitle');
            }
        });
    }, {
        placement: 'right'
    ...
    QueryBuilder.extend({
         ...
         */
        setFilters: function setFilters(delete_orphans, filters) {
            var self = this;
         ...
         */
        addFilter: function addFilter(new_filters, position) {
         ...
         * @param {boolean,optional} delete rules using old filters
         */
        removeFilter: function removeFilter(filter_ids, delete_orphans) {
            var filters = $.extend(true, [], this.filters);
    ...
    QueryBuilder.extend({
         ...
         */
        invert: function invert(node, options) {
    ...
    QueryBuilder.defaults({
        mongoOperators: {
            ...
            not_equal: function not_equal(v) {
                return { '$ne': v[0] };
            },
            in: function _in(v) {
                return { '$in': v };
            },
            not_in: function not_in(v) {
                return { '$nin': v };
            },
            less: function less(v) {
                return { '$lt': v[0] };
            },
            less_or_equal: function less_or_equal(v) {
                return { '$lte': v[0] };
            },
            greater: function greater(v) {
                return { '$gt': v[0] };
            },
            greater_or_equal: function greater_or_equal(v) {
                return { '$gte': v[0] };
            },
            between: function between(v) {
                return { '$gte': v[0], '$lte': v[1] };
            },
            not_between: function not_between(v) {
                return { '$lt': v[0], '$gt': v[1] };
            },
            begins_with: function begins_with(v) {
                return { '$regex': '^' + Utils.escapeRegExp(v[0]) };
            },
            not_begins_with: function not_begins_with(v) {
                return { '$regex': '^(?!' + Utils.escapeRegExp(v[0]) + ')' };
            },
            contains: function contains(v) {
                return { '$regex': Utils.escapeRegExp(v[0]) };
            },
            not_contains: function not_contains(v) {
                return { '$regex': '^((?!' + Utils.escapeRegExp(v[0]) + ').)*$', '$options': 's' };
            },
            ends_with: function ends_with(v) {
                return { '$regex': Utils.escapeRegExp(v[0]) + '$' };
            },
            not_ends_with: function not_ends_with(v) {
                return { '$regex': '(?<!' + Utils.escapeRegExp(v[0]) + ')$' };
            },
            is_empty: function is_empty(v) {
                return '';
            },
            is_not_empty: function is_not_empty(v) {
                return { '$ne': '' };
            },
            is_null: function is_null(v) {
                return null;
            },
            is_not_null: function is_not_null(v) {
                return { '$ne': null };
            }
        },
    ...
    QueryBuilder.extend({
         ...
         * @return {object}
         */
        getMongo: function getMongo(data) {
         ...
         * @return {object}
         */
        getRulesFromMongo: function getRulesFromMongo(data) {
         ...
         * @param data {object}
         */
        setRulesFromMongo: function setRulesFromMongo(data) {
    ...
    QueryBuilder.defaults({
        ...
        sqlRuleOperator: {
            ...
            'NOT BETWEEN': function NOTBETWEEN(v) {
        ...
        sqlStatements: {
            ...
            'numbered': function numbered(char) {
        ...
        sqlRuleStatement: {
            'question_mark': function question_mark(values) {
                var index = 0;
                return {
                    parse: function parse(v) {
                        return v == '?' ? values[index++] : v;
                    },
                    esc: function esc(sql) {
                        return sql.replace(/\?/g, '\'?\'');
                    }
                };
            },

            'numbered': function numbered(values, char) {
                if (!char || char.length > 1) char = '$';
            ...
            'named': function named(values, char) {
                if (!char || char.length > 1) char = ':';
    ...
    QueryBuilder.extend({
         ...
         * @return {object}
         */
        getSQL: function getSQL(stmt, nl, data) {
            data = data === undefined ? this.getRules() : data;
            nl = nl === true ? '\n' : ' ';
         ...
         * @return {object}
         */
        getRulesFromSQL: function getRulesFromSQL(data, stmt) {
         ...
         * @param data {object}
         */
        setRulesFromSQL: function setRulesFromSQL(data, stmt) {
    ...
    QueryBuilder.extend({
        updateDisabledFilters: function updateDisabledFilters(e) {
        ...
        clearDisabledFilters: function clearDisabledFilters(e) {
            var self = e ? e.builder : this;
        ...
        applyDisabledFilters: function applyDisabledFilters(e) {
            var self = e ? e.builder : this;
